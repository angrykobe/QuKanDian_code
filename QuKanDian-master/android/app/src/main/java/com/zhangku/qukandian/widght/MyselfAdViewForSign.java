...
/**
 * 自主广告视频列表原生展示   签到页自主广告展示模板
 */
public class MyselfAdViewForSign extends LinearLayout {
    ...
    public MyselfAdViewForSign(Context context) {
        this(context, null);
    }

    public MyselfAdViewForSign(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyselfAdViewForSign(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initUI(Context context) {
        LayoutInflater.from(getContext()).inflate(R.layout.view_myself_adview_for_sign, this);
        this.linkWebview = findViewById(R.id.linkWebview);
        this.myselfView = findViewById(R.id.myselfView);

        this.bottomRedView = findViewById(R.id.dialog_sign_gold_icon_layout);
        this.bottomRedIv = findViewById(R.id.dialog_sign_gold_icon);
        this.bgIV = findViewById(R.id.dialog_sign_img);
        this.titleTV = findViewById(R.id.dialog_sign_title);
    ...
    public void setData(final AdLocationBeans.AdLocationsBean.ClientAdvertisesBean bean) {
        if (bean.getAdType() == 3) { //1 自主广告 2 联盟广告 3 合作链接
            ...
            lp.height = DisplayUtils.dip2px(mContext, 63.0f);
