...
public class UserMessageFragment extends BaseLoadMoreFragment implements MessageAdapter.OnClickRefreshListener {
    ...
    public void loadData(Context context) {
        ...
        if (DBTools.getMessages().size() > 0) {
            LocalMessageBean bean = DBTools.getMessageDate(Constants.TYPE_NEW);
            if (null != bean) {
                try {
        ...
        if(null == mGetPlatformMessageProtocol){
            mGetPlatformMessageProtocol = new GetNewPlatformMessageProtocol(getContext(),date, Constants.TYPE_NEW, new BaseModel.OnResultListener<List<MessageBean>>() {
                @Override
...
//                        if(itemMessageBean.getType() == Constants.TYPE_NEW){
