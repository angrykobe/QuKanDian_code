...
public class SaiboZkNativeAd extends ZkNativeAdBase implements ZkNativeAd, AdDownObserver.OnAdDownListener  {
...
//    }
     ...
     * @return
     */
    public NiceNativeAdDataBeen makeNiceNativeAdDataBeen(SaiboResBean.ImpBean allBean) {
        ...
        try {
            if (allBean != null) {
                for (SaiboResBean.ImpBean.ImageBean imgUrl: allBean.getImage()) {
                    //添加imageUrl
                    if(TextUtils.isEmpty(niceBeen.getImageUrl()))
                        niceBeen.setImageUrl(imgUrl.getIurl());
                    //添加imageUrls
                    niceBeen.getImageUrls().add(imgUrl.getIurl());
                }
                for (SaiboResBean.ImpBean.WordBean wordBean: allBean.getWord()) {
                    switch (wordBean.getType()){//标题(1),广告语(2),广告描述(3)。
                        case 1:
                            niceBeen.setTitle(wordBean.getText());
                            break;
                        case 2:
                            break;
                        case 3:
                            //添加描述
                            niceBeen.setDescription(wordBean.getText());
                            break;
                    }
                }
            }
            ...
            //曝光监测地址
            if(allBean.getImptk() != null){
                niceBeen.setExp(allBean.getImptk());
            }
            //点击监测地址
            if(allBean.getClicktk() != null){
                niceBeen.setClc(allBean.getClicktk());
            }

            niceBeen.setClickUrl(allBean.getLink());// 点击行为地址
            niceBeen.setDeeplink(allBean.getDeeplink());
            niceBeen.setClickIsDown(allBean.getAction() == 2);
