...
/**
 * 自主广告视频列表原生展示  视频主页自主广告展示模板
 */
public class MyselfAdViewForVideoHome extends LinearLayout {
    ...
    public MyselfAdViewForVideoHome(Context context) {
        this(context, null);
    }

    public MyselfAdViewForVideoHome(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyselfAdViewForVideoHome(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initUI(Context context) {
        LayoutInflater.from(getContext()).inflate(R.layout.view_myself_adview_for_videolist, this);
...
//            });
            ...
            if (AdZhiZuNative.getRedState(bean) != 0) {
                lp.height = DisplayUtils.getScreenWidth(mContext) / 2 + DisplayUtils.dip2px(mContext, 40.0f);
            } else {
                lp.height = DisplayUtils.getScreenWidth(mContext) / 2;
            }
