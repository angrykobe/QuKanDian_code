...
public class ReadProgressAdapter extends BaseRecyclerViewAdapter<Object> {
    private int mType;
    private String mBelong;

    public ReadProgressAdapter(Context context, List<Object> beans, int type) {
        super(context, beans);
        mType = type;
        switch (mType) {
    ...
    protected RecyclerView.ViewHolder getViewHolder(ViewGroup parent, int type) {
        ...
        switch (type) {
            case TYPE_AD_VIEW://联盟广告
                viewHolder = new ReadProgressAdapter.NativAdHodler(LayoutInflater.from(mContext).inflate(R.layout.details_ad_view_right, parent, false));
                break;
            case TYPE_AD_VIEW_SELF://自主广告
                viewHolder = new ReadProgressAdapter.MyAdHolder(new MyselfAdViewForVideoDetail(mContext));
                break;
            case VIEW_TYPE_NORMAL://（视频，文章）列表
                viewHolder = new ReadProgressAdapter.MyViewHolder(LayoutInflaterUtils.inflateView(mContext, R.layout.item_details_adapter_view));
                break;
            case TYPE_EMPTY:
                viewHolder = new ReadProgressAdapter.EmptyViewHolder(new Space(mContext));
                break;
        }
        return viewHolder;
    }
    ...
    protected void bindView(RecyclerView.ViewHolder holder, int viewPosition, int dataIndex, final Object bean1) {
        if (holder instanceof ReadProgressAdapter.MyViewHolder && bean1 instanceof InformationBean) {
            ReadProgressAdapter.MyViewHolder view = (ReadProgressAdapter.MyViewHolder) holder;
        ...
        } else if (holder instanceof ReadProgressAdapter.NativAdHodler) {
            final ReadProgressAdapter.NativAdHodler view = (ReadProgressAdapter.NativAdHodler) holder;
...
//                        nativeAdInfo.onClick(mContext, view, nativeAdInfo.getAdsRuleBean());
        ...
        } else if (holder instanceof ReadProgressAdapter.MyAdHolder) {
