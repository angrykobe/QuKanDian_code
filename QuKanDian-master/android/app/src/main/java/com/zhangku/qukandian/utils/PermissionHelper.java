...
public class PermissionHelper {
    ...
    private final static int REQUEST_OPEN_APPLICATION_SETTINGS_CODE = 103;

    public final static int READ_CONTACTS_CODE = 104;
    ...
    public static boolean check(String permission) {
        ...
        //是否拥有权限
        if (ActivityCompat.checkSelfPermission(QuKanDianApplication.getmContext(), permission) != PackageManager.PERMISSION_GRANTED) {
            return false;
        }
        return true;
    }
    ...
    // 显示打开权限提示
    public static void showAlert(final Context context, String permissionName) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context).setTitle("权限申请")
                .setMessage("我们需要获取" + permissionName + "权限，否则将无法正常使用")
